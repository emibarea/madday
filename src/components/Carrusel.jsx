...
const Carrusel = () => {
  ...
  const carruselItems = comercios.flatMap((comercio, index) => [
    comercio,
    articulos[index],
  ]);
  return (
    <div className="z-0 h-[780px] w-full relative group">
      ...
      {carruselItems[currentIndex].imagenes?.carrusel ? (
        <div
          style={{
            backgroundImage: `url(${carruselItems[currentIndex].imagenes?.carrusel})`,
          }}
          className="bg-fixed w-full h-full bg-center bg-cover duration-1000"
        ></div>
      ) : (
        <div
          style={{
            backgroundImage: `url(${carruselItems[currentIndex].img})`,
          }}
          className="bg-fixed w-full h-full bg-center bg-cover duration-1000"
        ></div>
      )}
      ...
      <div className="absolute top-[40%] left-[10%] z-20 max-w-xs font-sans text-white sm:max-w-sm">
        <h1 className="text-4xl font-extrabold underline dark:text-white">
          {carruselItems[currentIndex].titulo}
        </h1>
        <p className="my-4 text-sm sm:text-lg overflow-ellipsis overflow-hidden max-h-48 rounded-lg">
          {carruselItems[currentIndex]?.descripcion}
        </p>
        {carruselItems[currentIndex].imagenes?.carrusel ? (
          <Link
            href={`comercios/${carruselItems[currentIndex].titulo}`}
